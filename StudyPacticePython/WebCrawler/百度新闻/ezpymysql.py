...
class Connection(object):
    ...
    def __init__(self, host, database, user=None, password=None,
        ...
        try:
            ...
            logging.error(f"Cannot connect to MySQL {self.host}", exc_info=True)

    def _ensure_connected(self):
        """如果客户端休息超过7小时，重新链接sql"""
        """Mysql by default closes client connections that are idle for 8 hours, but the client library does not report this fact until you try to perform a query and it fails.  Protect against this case by preemptively closing and reopening the connection  if it has been idle for too long (7 hours by default)."""
    ...
    def reconnect(self):
        """Closes the existing database connection and re-open it."""
        self.close()
